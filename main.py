...
class УчительКонтроллер:
    def __init__(self):
        self.сервис = УчительСервис()
        self.вью = УчительВью()

    def добавить_учителя(self):
        имя, предмет = self.вью.запросить_данные_учителя()
        учитель = self.сервис.добавить_учителя(имя, предмет)
        print(f"Учитель {учитель.имя} успешно добавлен.")

    def показать_всех_учителей(self):
        учителя = self.сервис.получить_учителей()
        self.вью.показать_учителей(учителя)

    def редактировать_учителя(self):
        id = self.вью.запросить_id_учителя()
        учитель = self.сервис.найти_учителя_по_id(id)
        if учитель:
            имя, предмет = self.вью.запросить_данные_учителя()
            if self.сервис.редактировать_учителя(id, имя, предмет):
                print(f"Учитель {id} успешно обновлен.")
            else:
                print(f"Не удалось обновить учителя {id}.")
        else:
            print(f"Учитель с ID {id} не найден.")

    def показать_учителя_по_id(self):
        id = self.вью.запросить_id_учителя()
        учитель = self.сервис.найти_учителя_по_id(id)
        self.вью.показать_учителя(учитель)

if __name__ == "__main__":
    контроллер = УчительКонтроллер()
    ...
    while True:
        ...
        if выбор == "1":
            контроллер.добавить_учителя()
        elif выбор == "2":
            контроллер.показать_всех_учителей()
        elif выбор == "3":
            контроллер.редактировать_учителя()
        elif выбор == "4":
            контроллер.показать_учителя_по_id()
        elif выбор == "5":
            break
        else:
